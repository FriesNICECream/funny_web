...
function Prism({
  ...
  useEffect(() => {
    ...
    const wX = (0.3 + randomValue() * 0.6) * RSX;
    const wY = (0.2 + randomValue() * 0.7) * RSY;
    const wZ = (0.1 + randomValue() * 0.5) * RSZ;
    const phX = randomValue() * Math.PI * 2;
    const phZ = randomValue() * Math.PI * 2;
    ...
    return () => {
      ...
      if (suspendWhenOffscreen) {
        const intersectionObserver = container.__prismIO;
